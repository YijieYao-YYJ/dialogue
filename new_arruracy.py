...
RESULTS_PATH = Path("results/rnn/rnn_results.json")
...
def prf(tp: int, p: int, g: int):
    """根据 TP / Pred / Gold 计算 Precision / Recall / F1。"""
...
def reciprocal_rank_at_k(pred_names, gold_names, k: int) -> float:
    """
    计算当前样本的 RR@K：
      - 在 pred_names[:K] 中找到第一个命中的 gold；
      - 若在位置 i 命中（0-based），则 RR = 1 / (i+1)；
      - 若没有命中任何 gold，则 RR = 0。
    """
    topk = pred_names[:k]
...
def evaluate():
    ...
    # 用于 MRR 的累积：只累积 RR 之和，最后再除以 samples
