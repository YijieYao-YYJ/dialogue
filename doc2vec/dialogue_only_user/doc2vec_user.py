...
# -------------------- Config --------------------
QUERIES_PATH   = Path("../../results/quiries/queries_test_user.json")
ENTITIES_PATH  = Path("../../results/entities/entities.json")
MODEL_PATH     = Path("doc2vec_dbow.bin")
OUT_PATH       = Path("../../results/doc2vec/doc2vec_user_results.json")
...
def train_or_load_model(entities, queries) -> Doc2Vec:
    ...
    if not tagged_all:
        raise RuntimeError("No training data found. Check entities.json and queries.json.")
...
def search_and_write(model: Doc2Vec, entities, queries):
    ...
    for q in queries:
        ...
        for dom in domains:
            ...
            mat     = dom_index[dom]["mat"]

            if mat.shape[0] == 0:
                per_domain.append({"domain": dom, "topk": []})
                continue
    ...
    print(f"Doc2Vec(PV-DBOW) 完成检索，写出 {len(results)} 条结果 -> {OUT_PATH}")

def main():
    if not ENTITIES_PATH.exists():
        raise FileNotFoundError("entities.json 不存在。")
    if not QUERIES_PATH.exists():
        raise FileNotFoundError("queries.json 不存在。")
