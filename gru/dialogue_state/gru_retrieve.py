...
OUT_PATH          = Path("../../results/gru/dialogue_state/gru_results.json")
...
def constraints_to_text(q: Dict[str, Any]) -> str:
    """
    把 query 里的 dialogue state(constraints) 转成一段查询字符串。
    设计：domain + slot + value 比如：
      restaurant pricerange cheap area centre food chinese hotel stars 4 area north ...
    """
